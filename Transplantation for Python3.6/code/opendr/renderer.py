...
class ColoredRenderer(BaseRenderer):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.camera:
            if self.overdraw:
                return common.dImage_wrt_2dVerts_bnd(color, visible, visibility, barycentric, self.frustum['width'], self.frustum['height'], int(self.v.r.size/3), self.f, self.boundaryid_image != 4294967295)
            else:
                return common.dImage_wrt_2dVerts(color, visible, visibility, barycentric, self.frustum['width'], self.frustum['height'], int(self.v.r.size/3), self.f)
...
class TexturedRenderer(ColoredRenderer):
    ...
    def wireframe_tex_coords(self):
        vvt = np.zeros((int(self.v.r.size/3),2), dtype=np.float32, order='C')
...
def _setup_camera(gl, cx, cy, fx, fy, w, h, near, far, view_matrix, k):
    ...
    if np.any(k):
        if not hasattr(gl, 'distortion_shader'):
            ...
            gl.ShaderSource(vs, 1, vs_source.encode(), len(vs_source)) # gl.ShaderSource(vs, 1, vs_source, len(vs_source))
        ...
        for idx, vname in enumerate(['k1', 'k2', 'p1', 'p2', 'k3', 'k4', 'k5', 'k6']):
            loc = gl.GetUniformLocation(gl.distortion_shader, vname.encode()) # loc = gl.GetUniformLocation(gl.distortion_shader, vname)
