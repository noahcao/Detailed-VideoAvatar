...
class Ch(object):
    ...
    def dr_wrt(self, wrt, reverse_mode=False, profiler=None):
        ...
        if len(drs)==0:
            ...
            if not np.any([isinstance(a, LinearOperator) for a in drs]):
                from functools import reduce # add
                result = reduce(lambda x, y: x+y, drs)
            else:
        ...
        # on average.
        if len(list(self._parents.keys())) != 1:# if len(self._parents.keys()) != 1:
    ...
    # 
    def __rtruediv__(self, other): return ch_ops.divide(x1=other, x2=self)
...
def depends_on(*dependencies):
    ...
    def _depends_on(func):
        ...
        @wraps(func)
        def with_caching(self, *args, **kwargs):
            func_name = func.__name__# func_name = func.func_name
